...
M.find_project_files = function(prompt_bufnr, hidden_files)
	...
	if cd_successful then
		vim.schedule(function()
			...
			require("lazyvim.util").telescope("files", { cwd = project_path, hidden = hidden_files })()
		end)
	end
end
...
M.browse_project_files = function(prompt_bufnr)
	...
	if cd_successful then
		vim.schedule(function()
			...
			vim.api.nvim_command("Neotree " .. vim.fn.fnameescape(project_path))
		end)
	end
end
...
M.recent_project_files = function(prompt_bufnr)
	...
	if cd_successful then
		vim.schedule(function()
			...
			require("lazyvim.util").telescope_old_files({ cwd_only = true })
		end)
	end
end
